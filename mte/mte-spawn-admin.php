...
class MySQLtabledit {
	...
	*/
    ...
    $tubs = "SELECT SUM(tubs) AS tubs FROM spawn $where_search"; 
    $contam = "SELECT SUM(contam) AS contam FROM spawn $where_search"; 
    $result = mysqli_query($db2, $tubs);
    $result2 = mysqli_query($db2, $contam); 
    ...
    { 
        ...
        $total = $row['tubs'] - $row2['contam'];
          
           if ($row) 
              { 
                 printf("<div align=\"left\" style=\"padding-left: 4px;\">Total number of tubs : " . $row['tubs']);
                 printf("<div align=\"left\">Total number of contaminated tubs : " . $row2['contam']); 
                 printf("<div align=\"left\">Total number of active tubs : " . $total); 
              }  
...
include ("fetchinfo.php");
	...
	//LOG END
		...
		$this->javascript = "
			...
			function archive_confirm(id) {
				if (confirm('{$this->text['Archive']} record {$this->show_text_listview[$this->primary_key]} ' + id + '...? Only archive if you are harvesting or you have edited the contam field.')) {
	...
	##################
		...
		while($fetch = mysqli_fetch_array($query)){
			mysqli_query($this->mysqli, "INSERT INTO `archive_spawn` VALUES('', '$fetch[date]', '$fetch[username]', '$fetch[strain]', '$fetch[location]', '$fetch[label]', '$fetch[tubs]', '$fetch[contam]', '$fetch[fruit]')") or die(mysqli_error($this->mysqli));
			}
