...
class MySQLtabledit {
	...
	*/
    ...
    $query = "SELECT id FROM harvest $where_search";  
    $result = mysqli_query($db2, $query); 
    $dry = "SELECT SUM(dry) AS dry FROM harvest $where_search";
	 $result2 = mysqli_query($db2, $dry);
	 $available = "SELECT SUM(available) AS available FROM harvest $where_search"; 
	 $result3 = mysqli_query($db2, $available);
    ...
    { 
        ...
        $total = $row2['dry'] - $row3['available'];
          
           if ($row) 
              { 
                 printf("<div align=\"left\" style=\"padding-left: 4px;\">Total number of harvests : " . $row); 
                 printf("<div align=\"left\" style=\"padding-top: 2px;\">Total number dry : " . $row2['dry']);
                 printf("<div align=\"left\" style=\"padding-top: 2px;\">Total number available : " . $row3['available']);
...
include ("fetchinfo.php");
	if ($_GET['s'] && $_GET['f']) {
		$page_title = "Harvest"; 
		$sql2 = "INSERT INTO LogSearch (user, search, type, page, IP) VALUES ('$currentuser', '$in_search', '$type', '$page_title', '$ip')";
					...
					if (in_array($key, $this->fields_in_list_view,true)) {
						if ($count == 1) {
							...
							if ($_GET['sort'] == $key && $_GET['ad'] == 'a') {
								$sort_image = "</div><img src='$this->url_base/images/sort_a.png' style='width:9px;height:8px' alt=''>";
								$ad = 'd';
							}
						...
						//&nbsp;&nbsp;<a href='javascript:void(0)' onclick='archive_confirm($value)' title='Archive {$this->show_text_listview[$key]} $value'><img src='$this->url_base/images/archive.png' class='icons' title='{$this->text['Archive']}'></a>
			...
			<tr>
				...
				<td nowrap style='text-align: right'>
					<form method=get action='$this->url_script' style='padding: 15px'>
						...
						<input type='submit' value='Go' onclick=\"welcome()\" style='width:40px;min-width:40px;border: 0;'>
					</form>
		 	"; 	
	...
	##################
		...
		while($fetch = mysqli_fetch_array($query)){
			mysqli_query($this->mysqli, "INSERT INTO `archive_harvest` VALUES('$fetch[date]', '$fetch[username]', '$fetch[strain]', '$fetch[location]', '$fetch[label]', '$fetch[flush]', '$fetch[wet]', '$fetch[dry]')") or die(mysqli_error($this->mysqli));
			}

		if (mysqli_query($this->mysqli,"DELETE FROM $this->table WHERE `$this->primary_key` = '$in_id'")) {
			$this->content_archived = "
				<div style='width: $this->width_editor'>
					<div style='padding: 10px; color:#fff; background: #FF8000; font-weight: bold'>Record {$this->show_text[$this->primary_key]} $in_id {$this->text['archived']}</div>
				</div>
			";
			$this->show_list();
		}
