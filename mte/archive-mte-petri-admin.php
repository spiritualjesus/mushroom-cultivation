...
class MySQLtabledit {
	...
	*/
    ...
    $dishes = "SELECT SUM(dishes) AS dishes FROM archive_petri $where_search"; 
    $contam = "SELECT SUM(contam) AS contam FROM archive_petri $where_search"; 
    $result = mysqli_query($db2, $dishes);
    ...
    { 
        ...
        $total = $row['dishes'] - $row2['contam'];
          
           if ($row) 
              { 
                 printf("<div align=\"left\" style=\"padding-left: 2px; padding-top: 2px;\">Total number of archived petri dishes : " . $row['dishes']);
                 printf("<div align=\"left\" style=\"padding-top: 2px;\">Total number of contaminated petri dishes : " . $row2['contam']); 
                 printf("<div align=\"left\" style=\"padding-top: 2px;\">Total number of petri dishes spawned successfully: " . $total); 
              }  
...
##################
	...
	##################
		...
		while($fetch = mysqli_fetch_array($query)){
			mysqli_query($this->mysqli, "INSERT INTO `archive_petri` VALUES('$fetch[date]', '$fetch[username]', '$fetch[strain]', '$fetch[location]', '$fetch[label]', '$fetch[description]', '$fetch[dishes]', '$fetch[contam]')") or die(mysqli_error($this->mysqli));
			}
