...
class MySQLtabledit {
	...
	*/
    ...
    { 
	 ...
	 $bags = "SELECT COUNT(*) AS bags FROM archive_grain $where_search"; 
    $contam = "SELECT SUM(contam) AS contam FROM archive_grain $where_search";
    $result = mysqli_query($db2, $bags);
    ...
    { 
        ...
        $total = $row['bags'] - $row2['contam'];
          
           if ($row) 
              { 
                 printf("<div align=\"left\" style=\"padding-left: 4px; padding-top: 1px;\">Total number of archived grain bags : " . $row['bags']);
                 printf("<div align=\"left\" style=\"padding-top: 2px;\">Total number of contaminated grain bags : " . $row2['contam']); 
                 printf("<div align=\"left\" style=\"padding-top: 2px;\">Total number of grain bags spawned successfully: " . $total); 
              }  
...
//end
		...
		$this->javascript = "
			...
			function archive_confirm(id) {
				if (confirm('{$this->text['Archive']} record {$this->show_text_listview[$this->primary_key]} ' + id + '...?')) {
	...
	##################
		...
		while($fetch = mysqli_fetch_array($query)){
			mysqli_query($this->mysqli, "INSERT INTO `archive_grain` VALUES('$fetch[date]', '$fetch[username]', '$fetch[strain]', '$fetch[location]', '$fetch[label]', '$fetch[source]', '$fetch[bags]', '$fetch[contam]')") or die(mysqli_error($this->mysqli));
			}
...
###################
	function harvest_rec() {
	###################

		if (isset ($_GET['id'])) $in_id = $_GET['id'];

		# edit or new?
		if ($_GET['mte_a'] == 'Harvest') $edit=1;
		else $edit = 0;
