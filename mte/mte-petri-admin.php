...
class MySQLtabledit {
	...
	*/
    ...
    $dishes = "SELECT SUM(dishes) AS dishes FROM petri $where_search"; 
    $contam = "SELECT SUM(contam) AS contam FROM petri $where_search"; 
    ...
    { 
        ...
        $total = $row['dishes'] - $row2['contam'];
           ...
           if ($row) 
              { 
                 printf("<div align=\"left\" style=\"padding-left: 4px;\">Total number of petri dishes : " . $row['dishes']);
                 printf("<div align=\"left\">Total number of contaminated petri dishes : " . $row2['contam']); 
                 printf("<div align=\"left\">Total number of active petri dishes  : " . $total); 
              }  
...
//end
		...
		$this->javascript = "
			...
			function archive_confirm(id) {
				if (confirm('{$this->text['Archive']} record {$this->show_text_listview[$this->primary_key]} ' + id + '...? Only archive if you are inoculating or dish(es) have contam.')) {
