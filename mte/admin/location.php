...
class MySQLtabledit {
	...
	*/
    ...
    $query = "SELECT id FROM location $where_search"; 
    ...
    { 
        $row = mysqli_num_rows($db2); 
           ...
           if ($row) 
              { 
                 printf("<div align=\"left\" style=\"padding-left: 2px;\">Total number of locations : " . $row); 
              } 
...
//end
		...
		if (mysqli_num_rows($result)>0) {
			...
			while ($rij = mysqli_fetch_assoc($result)) {
				...
				foreach ($rij AS $key => $value) {
					...
					//JAKE CHANGE NUM TO TEXT
					if (($value == 1) && ($key !== "id")) { $value = "Yes"; } elseif (empty($value) && ($key !== "id")) { $value = "No"; }
					//END
					...
					if (in_array($key, $this->fields_in_list_view,true)) {
						...
						if ($key == $this->primary_key) {
							if (($key == "id") && (($value == 1) || ($value == 2))){
							$buttons = "<td nowrap><a href='?$query_string&mte_a=edit&id=$value' title='Edit {$this->show_text_listview[$key]} $value'><img src='$this->url_base/images/edit.png' class='icons' title='{$this->text['Edit']}'></a></td>";
							} else {
			...
			<tr>
				...
				<td nowrap style='text-align: right'>
					<form method=get action='$this->url_script' style='padding: 15px'>
						...
						<input type='submit' value='Go' style='width:40px;min-width:40px;border: 0;'>
					</form>
		 	"; 	
		...
		$this->javascript = "
			function del_confirm(id) {
				if (confirm('{$this->text['Delete']} user {$this->show_text_listview[$this->primary_key]} ' + id + '...?')) {
	...
	##################
		...
		$result = (mysqli_query($this->mysqli,"SELECT `username` FROM $this->table WHERE `$this->primary_key` = '$in_id'")); 
		while($row = mysqli_fetch_array($result)) { $username = $row['username']; }
			
		if (mysqli_query($this->mysqli,"DELETE FROM $this->table WHERE `$this->primary_key` = '$in_id'")) {
				//JAKE LOG	
				include ("fetchinfo.php");
			$sql2 = "INSERT INTO LogAccountDel (user, admin, IP) VALUES ('$username', '$currentuser', '$ip')";
       if (mysqli_query($this->mysqli, $sql2)) { }
			//LOG END
			
			$this->content_deleted = "
				<div style='width: $this->width_editor'>
					<div style='padding: 10px; color:#fff; background: #FF8000; font-weight: bold'>User {$this->show_text[$this->primary_key]} $username {$this->text['deleted']}</div>
				</div>
			";
			$this->show_list();
		}
			...
			$this->content = "
				...
				"</div><a href='$this->url_script'>List users...</a>
			</div>";
		}
