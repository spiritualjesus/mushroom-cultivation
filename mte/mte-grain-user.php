...
class MySQLtabledit {
	...
	*/
	...
	####################
		...
		if ($_GET['s'] && $_GET['f']) {
			...
			if ($in_search_field == $this->primary_key) {
				$where_search = "WHERE $in_search_field = '$in_search' AND strain NOT LIKE '%*%' ESCAPE '*'";
			}
			else {
				$where_search = "WHERE $in_search_field LIKE '%$in_search%' AND strain NOT LIKE '%*%' ESCAPE '*'";
			}
		}
		// count rows
    ...
    $query = "SELECT id FROM grain $where_search";  
    ...
    { 
        $row = mysqli_num_rows($result); 
           ...
           if ($row) 
              { 
                 printf("<div align=\"left\" style=\"padding-left: 2px;\">Total number of grain bags : " . $row); 
              } 
